...
def extract_combined_features(
    ...
    overall_schema_type = "complex" if t1["schema_type"] == "complex" or t2["schema_type"] == "complex" else "simple"
    ...
    if _df_cache is None:
        ...
        if "total_latency_s" not in df.columns:
            if "match_generation_time" in df.columns and "merge_generation_time" in df.columns:
                df["total_latency_s"] = df["match_generation_time"].fillna(0) + df["merge_generation_time"].fillna(0)
        _df_cache = df
    return _df_cache
...
def _patch_model(model):
    """
    Recursively patch for scikit-learn version mismatch.
    Fixes: 'SimpleImputer' object has no attribute '_fill_dtype'
    """
    ...
    def patch_obj(obj):
        ...
        if isinstance(obj, ColumnTransformer):
            for _, transformer, _ in obj.transformers_:
                patch_obj(transformer)
                
        # Handle cases where attributes might contain more estimators
        if hasattr(obj, 'estimator'):
            patch_obj(obj.estimator)
...
def _build_eval_frame(
    ...
    default_out_tokens = float(df["output_tokens"].median()) if "output_tokens" in df.columns else 500.0
    base = {
        "schema_type": schema_type,
